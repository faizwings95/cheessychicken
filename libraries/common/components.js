						...
						if(typeof(froalaEditor)!='undefined' &&  typeof(froalaEditor.textAlign)!='undefined'){
							...
							if(froalaEditor.textAlign == 'right' || froalaEditor.textAlign == 'center'){
								...
								if($item.attr('data-width')>_width) {
									...
									prxy.items[prx.items.helper.getIndexFromId(item.id)].left = prxy.items[prx.items.helper.getIndexFromId(item.id)].left + diff;
								...
								$item.width(_width).height(_height).css('left',prxy.items[prx.items.helper.getIndexFromId(item.id)].left);
						...
						onClick: function(item) {
							...
							if(isAutoResize && !convertToFixed) {
                                ...
                                if(convertToFixed){//in case autoresize true but using lorem ipsum is going to create a very wide textbox, then convert to fixed
                                    ...
                                    $item.width(_width);
                                    item.width = _width;
                                    prxy.items[prx.items.helper.getIndexFromId(item.id)].autoResize = false;
                                    $iframeRichTextEL.width(_width).height(_height);
								}else{
                                    $item.height(_height);
                                    $iframeRichTextEL.width(item.width).height(_height);
...
prx.types.rectangle = {
	...
	,onDisplay: function(item,containerid,symbol) {
		...
		cR += '</style>';
		cR += prx.items.getComponentPrependDivs(item, containerid, symbol);
		cR += '<div id="rec-' + _id + '" class="inner-rec liveUpdate-backgroundColor-background-color changeProperty-backgroundColor changeProperty-backgroundColor-background-color changeProperty-borderColor changeProperty-borderColor-border-color liveUpdate-borderColor-border-color">';
...
prx.types.tooltip = {
	...
	,onDisplay: function(item,containerid,symbol) {
		...
		cR += '<div class="tooltip-text liveUpdate-textColor-color changeProperty-textColor changeProperty-textSize changeProperty-textFont"> <span data-editableproperty="text">'+prx.componentsHelper.getProp(item.text,'text-textarea')+'</span></div>'
		cR += '</div>';
		cR += '</div>';
